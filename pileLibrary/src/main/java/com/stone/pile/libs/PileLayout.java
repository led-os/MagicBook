...
public class PileLayout extends ViewGroup {
    ...
    public PileLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取布局配置 1. 最小滑动距离(mTouchSlop) 2. 最大滑动加速度(mMaximumVelocity)
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //每个childView 的宽度
        everyWidth = (int) ((width - getPaddingLeft() - getPaddingRight() - interval * 7) / displayCount);
        //每个childView 的高度
        ...
        //确定每个View 的位置
        // 一共确定了 9个 初始位置
        // scrollDistanceMax 滑动间隔的最长距离
        // 根据 interval view 的间隔
        if (originX.size() == 0) {
            int position0 = 0;
    ...
    private void requireScrollChange(int dx) {
        ...
        if(getChildAt(3) != null){
             currentPosition = Integer.parseInt(getChildAt(3).getTag().toString());
        }
